...
//    Modifying the response entity to use the ResponseEntity<>
    ...
    //Get one book
    @GetMapping("/{Id}")
    public ResponseEntity<HashMap<String, Object>> getBookById(@PathVariable Long Id, HttpServletRequest request) throws NoBookFoundException {
    ...
    //By published year
    @GetMapping("/books")
    public Page<Book> getBooksByPublishedYear(
        ...
        return bookService.getFilteredBooks(year, title,author,page, size, sortBy);
    }
//    filtering book by author and title
    @GetMapping("/filtered/author/title")
    public Page<Book>FetchBookByAuthorAndTitle(
            @RequestParam(defaultValue = "") String author,
            @RequestParam(defaultValue = "") String title,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "30") int size)
    {
     return bookService.FetchBookByAuthorAndTitle(author,title,page,size);
    }
    //   Create books
